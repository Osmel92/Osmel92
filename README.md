# 👋 ¡Hola, soy Osmel92! | Técnico en Informática & Desarrollador Web


![Encabezado de Osmel Alcina - Tu Aliado Tecnológico](header/header.webp)


<p align="center">
  <img src="https://img.shields.io/badge/WordPress-21759B?style=for-the-badge&logo=wordpress&logoColor=white" alt="WordPress" />
  <img src="https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white" alt=".NET" />
  <img src="https://img.shields.io/badge/Adobe%20Photoshop-31A8FF?style=for-the-badge&logo=adobephotoshop&logoColor=black" alt="Photoshop" />
  <img src="https://img.shields.io/badge/Adobe%20Illustrator-FF9A00?style=for-the-badge&logo=adobeillustrator&logoColor=black" alt="Illustrator" />
</p>

---

### 🚀 Sobre Mí

Desde que inicié mi camino en la tecnología, descubrí que mi verdadera pasión es usar los sistemas para **resolver problemas reales**. Empecé optimizando hardware y hoy, como **Técnico en Informática**, fusiono el soporte de infraestructura con el desarrollo de software y el diseño digital.

Mi objetivo es ser el aliado tecnológico que se encarga de los bits, los píxeles y el código, permitiendo que los pequeños negocios y emprendedores se digitalicen y funcionen al 100%.

* 👀 **Intereses:** Desarrollo web y diseño gráfico.
* 🌱 **Aprendizaje continuo:** Siempre explorando nuevas herramientas y tecnologías para expandir mi stack profesional.

---

### 🛠️ Mi Stack Tecnológico

| Área | Tecnologías y Herramientas |
| :--- | :--- |
| **Desarrollo Web** | `WordPress` `WooCommerce` `HTML5` `CSS3` |
| **Programación & BD** | `Visual Basic .NET` `C#` `SQL` |
| **Diseño & Contenido** | `Adobe Photoshop` `Adobe Illustrator` `Canva` |
| **Soporte & Infraestructura** | Mantenimiento preventivo/correctivo, Repotenciación de hardware |
| **Presencia Local** | Optimización de Google Business Profile |

---

### 📊 Mis Pilares de Trabajo

* **Desarrollo Web Eficiente:** Creación de páginas promocionales y tiendas online enfocadas en convertir visitas en ventas.
* **Soluciones de Software a Medida:** Programación de herramientas específicas que aseguran la continuidad y automatización de los negocios.
* **Diseño Digital de Alto Impacto:** Identidad visual y contenido para redes sociales que potencian la visibilidad de las marcas.

---

### 📫 Conéctate conmigo

* 💼 Puedes ver mis proyectos y trayectoria en [LinkedIn](www.linkedin.com/in/osmel-alcina-139606134)
* 💬 ¿Tienes un proyecto en mente o necesitas soporte técnico? ¡Escríbeme!

---
<p align="center">
  <i>"Me encargo de la tecnología para que tú solo te preocupes por hacer crecer tu negocio."</i>
</p>

